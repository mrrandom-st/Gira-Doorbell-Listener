...
#define SENSOR_TIMEOUT_MS 15000UL
...
float parseJsonFloat2(const String& json, const String& keyA, const String& keyB, float def = 0.0f) {
  int i = findJsonValueStart(json, keyA);
  if (i >= 0) return parseJsonFloat(json, keyA, def);
  i = findJsonValueStart(json, keyB);
  if (i >= 0) return parseJsonFloat(json, keyB, def);
  return def;
}

int parseJsonInt2(const String& json, const String& keyA, const String& keyB, int def = 0) {
  int i = findJsonValueStart(json, keyA);
  if (i >= 0) return parseJsonInt(json, keyA, def);
  i = findJsonValueStart(json, keyB);
  if (i >= 0) return parseJsonInt(json, keyB, def);
  return def;
}
...
void onDoorbellTriggeredFromJson(const String& json) {
  ...
  state.ratio = parseJsonFloat2(json, "ratio", "pk", state.ratio);
...
void onStatusFromJson(const String& json) {
  ...
  state.bandRatioMin  = parseJsonFloat2(json, "bandRatioMin", "peakThr", state.bandRatioMin);
  state.bandDomMin    = parseJsonFloat2(json, "bandDomMin", "domThr", state.bandDomMin);
  state.tfNeed        = parseJsonInt(json, "tfNeed", state.tfNeed);
  state.cooldownMs    = (uint32_t)parseJsonInt2(json, "cool", "cooldownMs", (int)state.cooldownMs);
...
void drawHeaderDynamic() {
  ...
  state.sensorAliveByTimeout = (ageMs < SENSOR_TIMEOUT_MS);
...
void loop() {
  ...
  if (millis() - lastSensorTimeoutCheck > 1000) {
    ...
    uint32_t now = millis();
    bool alive = (state.lastStatusMs != 0 && (now - state.lastStatusMs) < SENSOR_TIMEOUT_MS);
    // Wenn sich das ändert: redraw
    if (alive != state.sensorAliveByTimeout) {
      state.sensorAliveByTimeout = alive;
      state.dirtyHeader = true;
      state.dirtyStatus = true;
      if (!alive) Serial.println("⚠ Sensor timeout - no recent status");
    }
  }
